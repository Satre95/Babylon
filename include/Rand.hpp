...
//  Babylon
//
//  Created by Siddharth Atre on 12/19/17.
//
...
public:
    ...
    //! returns a random glm::vec3 that represents a point on the unit sphere
    glm::vec3 nextVec3()
    {
        float phi = nextFloat( (float)M_PI * 2.0f );
        float costheta = nextFloat( -1.0f, 1.0f );
        
        float rho = glm::sqrt( 1.0f - costheta * costheta );
    ...
    //! returns a random glm::vec2 that represents a point on the unit circle
    glm::vec2 nextVec2()
    {
    ...
    float nextGaussian()
    {
        if( mHaveNextNextGaussian ) {
            ...
            float m = glm::sqrt(-2.0f * glm::log(s)/s);
